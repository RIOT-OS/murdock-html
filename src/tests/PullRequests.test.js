...
const server = setupServer(
    rest.get('/api/pull_requests', (req, res, ctx) => {
        return res(ctx.json(
            {
                "building": [
                    {
...
const wsServer = new WS("ws://localhost:1234/status");
...
test('fetch and display pull requests', async () => {
    ...
    await waitFor(() => wsServer.send('{"cmd": "reload_prs"}'));
    await waitFor(() => screen.queryByText((content, element) => {
        return element.className === "card m-2 border-info";
    }));
    expect(screen.getByText((content, element) => {
    ...
    await waitFor(() => wsServer.send('{"cmd" : "prstatus", "prnum" : "16621", "status" : {"status": "working"}}'));
